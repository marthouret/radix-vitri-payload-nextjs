...
interface FonctionSimplifiee {
  // Juste le nom de la fonction du Verrier ou de la Personnalité, pour l'affichage
  nom: string
}

interface LieuSimplifie {
  // Juste les coordonnées et nom pour la carte/popup
...
interface VerrerieSimplifiee {
  // Juste ce dont on a besoin de la verrerie pour la carte
  id: string
  slug: string
  nomPrincipal: string
...
function transformerEngagementEnFriseItem(engagement: EngagementPourCarte): FriseItem {
  const {
    ...
    verrerie,
    fonctionVerrier,
    fonctionPersonnalite,
    typeEngagement,
    dateDebutStructurée,
    descriptionEngagement,
  } = engagement
  ...
  if (
    ...
    typeof fonctionPersonnalite === 'object' &&
    fonctionPersonnalite?.nom
  ) {
    cardTitleText = fonctionPersonnalite.nom
  } else if (
    typeEngagement === 'metier_verrier' &&
    typeof fonctionVerrier === 'object' &&
    fonctionVerrier?.nom
  ) {
  ...
  if (typeof verrerie === 'object' && verrerie !== null) {
    ...
    cardSubtitleText = localisationVerrerie
      ? `${nomVerrerie} (${localisationVerrerie})`
      : nomVerrerie
    ...
    if (verrerieObj.imagesEtMedias && verrerieObj.imagesEtMedias.length > 0) {
      ...
      if (premiereImage && premiereImage.url) {
        imagePourCarte = {
          ...
          alt:
            premiereImage.alt || premiereImage.filename || `Image de ${verrerieObj.nomPrincipal}`,
        }
      }
    }
  } else {
  ...
  const timelineContent = (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden flex flex-col h-full text-left no-underline">
      {' '}
      {/* Classes de base de votre carte */}
      {imagePourCarte && imagePourCarte.url ? (
        <div className="relative w-full h-40">
          {' '}
          {/* Hauteur légèrement réduite pour la frise, ex: h-40 (10rem) ou h-32 (8rem). À ajuster. */}
          <Image
      ...
      <div className="p-6 flex flex-col flex-grow">
        {' '}
        {/* Padding un peu réduit (p-4 au lieu de p-6) */}
        {urlLien ? (
          ...
          <h4 className="font-serif font-semibold text-base text-blueGray-800 mb-1 line-clamp-2">
            {cardTitleText}
          </h4>
        )}
        {cardSubtitleText && (
          <p className="text-blueGray-800 font-sans text-md font-semibold mt-1 mb-2 line-clamp-2">
            {cardSubtitleText}
          </p>
        )}
        {cardDetailedTextValue && (
          <p className="text-xs text-blueGray-600 font-sans line-clamp-6 flex-grow">
            {/* line-clamp-4 pour plus de texte */}
            {cardDetailedTextValue}
          </p>
        )}
      </div>
...
function transformerEvenementBiographiqueEnFriseItem(
  event: EvenementBiographiqueFromAPI,
): FriseItem {
  const {
    id,
    typeEvenement,
  ...
  const cardSubtitleText =
    lieu && typeof lieu === 'object' ? lieu.nomDuLieu || lieu.villeOuCommune : undefined
  ...
  const timelineContent = (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden flex flex-col h-full text-left no-underline p-4">
      ...
      {cardDetailedTextValue && (
        <p className="text-xs text-blueGray-600 font-sans line-clamp-4 flex-grow">
          {cardDetailedTextValue}
        </p>
      )}
    </div>
...
async function fetchFullLieu(id: string): Promise<LieuType | null> {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Erreur API Payload pour Lieu ID ${id} (${response.status}): ${await response.text()}`,
      )
      return null
    }
...
async function getVerrier(slug: string): Promise<VerrierFromAPI | null> {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Erreur API Payload pour verrier (${response.status}): ${await response.text()}`,
      )
      return null
    }
    ...
    if (verrier.id) {
      ...
      if (engagementsResponse.ok) {
        ...
        console.log(
          `[getVerrier] Engagements récupérés pour ${verrier.nomComplet}:`,
          verrier.engagements.length,
        )
      } else {
        console.error(
          `Erreur API Payload pour les engagements du verrier ${verrier.id} (${engagementsResponse.status}): ${await engagementsResponse.text()}`,
        )
        verrier.engagements = []
      }
    } else {
    ...
    if (verrier.id) {
      ...
      if (evenementsResponse.ok) {
        ...
        console.log(
          `[getVerrier] Événements Biographiques récupérés pour ${verrier.nomComplet}:`,
          verrier.evenementsBiographiques?.length || 0,
        )
      } else {
        console.error(
          `Erreur API Payload pour les événements biographiques du verrier ${verrier.id} (${evenementsResponse.status}): ${await evenementsResponse.text()}`,
        )
...
export default async function VerrierPage({ params }: { params: any }) {
  ...
  const nomCompletAffichage =
    verrier.nomComplet || `${verrier.prenom || ''} ${verrier.nom || ''}`.trim() || 'Verrier Inconnu'

  // --- PRÉPARATION DES DONNÉES POUR LA FRISE CHRONOLOGIQUE ---
  const engagementsFrise: FriseItem[] =
    verrier.engagements?.map(transformerEngagementEnFriseItem) || []
  const evenementsBioFrise: FriseItem[] =
    verrier.evenementsBiographiques?.map(transformerEvenementBiographiqueEnFriseItem) || []
  ...
  const mapPointsEngagement: MapPoint[] = (() => {
    ...
    const engagementsByVerrerieId: Record<
      string,
      {
    ...
    for (const eng of verrier.engagements) {
      ...
      const lieuPrincipalDoc =
        verrerieDoc.lieuPrincipal && typeof verrerieDoc.lieuPrincipal === 'object'
          ? verrerieDoc.lieuPrincipal
          : null
      ...
      if (
        ...
        eng.fonctionVerrier &&
        typeof eng.fonctionVerrier === 'object'
      ) {
        ...
        eng.fonctionPersonnalite &&
        typeof eng.fonctionPersonnalite === 'object'
      ) {
    ...
    return Object.values(engagementsByVerrerieId).map((data) => {
      const point: MapPoint = {
        // Assigner explicitement au type MapPoint
  ...
  console.log(
    '[VerrierPage] mapPointsEngagement calculés:',
    JSON.stringify(mapPointsEngagement.slice(0, 2), null, 2),
  )
        ...
        <div className="space-y-12 md:space-y-16">
          ...
          <section id="biographie">
            ...
            {verrier.notesBiographie && Object.keys(verrier.notesBiographie).length > 0 ? (
              <div className="prose prose-bluegray lg:prose-lg max-w-4xl mx-auto">
                {' '}
                {/* Prose uniquement pour le contenu */}
                <ArticleContentRenderer content={verrier.notesBiographie} />
              </div>
            ) : (
              <p className="italic text-blueGray-500 max-w-4xl mx-auto">
                Aucune note biographique.
              </p>
            )}
          </section>
          {/* Section Frise Chronologique */}
          <section id="frise-chronologique" className="mt-12 max-w-screen-xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-blueGray-800 mb-6 border-b-2 border-blueGray-200 pb-3 font-serif max-w-4xl mx-auto">
              Frise chronologique
            </h2>
            {friseItems.length > 0 ? (
              ...
              <p className="italic text-blueGray-500 max-w-4xl mx-auto">
                Aucun événement à afficher dans la frise pour ce verrier.
              </p>
            )}
          </section>
          ...
          <section id="parcours-professionnel" className="mt-12">
            {mapPointsEngagement.length > 0 ? (
              <div className="h-[450px] md:h-[550px] lg:h-[600px] bg-gray-200 rounded-lg shadow-md max-w-5xl mx-auto">
                {' '}
                {/* Hauteur augmentée et largeur max pour centrer */}
                <VerrerieMap
              ...
              <p className="italic text-blueGray-500 max-w-4xl mx-auto">
                Aucun engagement dans des verreries avec localisation connue pour ce verrier.
              </p>
            )}
          </section>

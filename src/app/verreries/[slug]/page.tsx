...
interface DateStructuree {
  // Pour les dates de début/fin détaillées
  anneeSort?: number
  moisSort?: number
...
interface PeriodeVerriereData {
  // Nouvelle interface pour l'objet periodeVerriere
...
interface SourceBibliographique {
  ...
  auteur?: string
  url?: string
  detailsPublication?: string
  citationOuExtrait?: string
  notesSource?: string
}
    ...
    <nav className="mb-4">
      <ul className="flex justify-center items-center gap-x-6 border-b border-blueGray-200 pb-2 list-none p-0">
        <li>
          <a
            href="#histoire"
            className="font-semibold text-blueGray-600 no-underline hover:text-gold transition-colors text-base"
          >
            {' '}
            {/* text-base ici */}
            Histoire
...
const BackToTop = () => {
  return (
    <div className="mt-4 pt-4 border-t border-blueGray-200">
      <a
        href="#"
        className="font-semibold text-blueGray-600 hover:text-gold transition-colors text-sm"
      >
        ↑ Retour en haut
      </a>
    </div>
...
const SuggestionCard: React.FC<{ verrerie: VerrerieType }> = ({ verrerie }) => {
  ...
  const imageUrl = image?.url
    ...
    : 'https://placehold.co/600x400.png?text=Image+N/A'
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="text-xl font-semibold text-blueGray-800 mb-2 font-serif group-hover:text-gold transition-colors">
          {name}
        </h3>
        {periodeAffichee && (
          <p className="text-blueGray-500 font-sans text-sm mb-4 flex-grow">{periodeAffichee}</p>
        )}
...
async function fetchFullRelatedDoc(
  collectionSlug: string,
  id: string | number,
): Promise<any | null> {
  try {
    const apiUrl = `${payloadUrl}/api/${collectionSlug}/${id}?depth=0&locale=fr&fallback-locale=fr`
    const response = await fetch(apiUrl, { cache: 'no-store' })
    if (!response.ok) {
      console.error(
        `[fetchRelatedDoc] Erreur API pour ${collectionSlug} ID ${id} (${response.status}): ${await response.text()}`,
      )
      return null
    }
    return await response.json()
  } catch (error) {
    console.error(`[fetchRelatedDoc] Exception pour ${collectionSlug} ID ${id}:`, error)
    return null
  }
}
...
async function getVerrerie(slug: string): Promise<VerrerieType | null> {
  try {
    ...
    if (!verrerieResponse.ok) {
      console.error(
        `Erreur API Payload (${verrerieResponse.status}): ${await verrerieResponse.text()}`,
      )
    ...
    if (verrerie.fondateurs && verrerie.fondateurs.length > 0) {
      const populatedFondateurs = await Promise.all(
        verrerie.fondateurs.map(async (personOrId) => {
    ...
    console.error('[getVerrerie]', error)
    return null
  }
}
...
export default async function VerreriePage({ params }: { params: any }) {
  ...
  console.log(
    "Données brutes pour la verrerie (nombre d'engagements) :",
    JSON.stringify(verrerie?.engagements?.length, null, 2),
  )
  ...
  console.log(
    "Données brutes pour la galerie (nombre d'images) :",
    JSON.stringify(verrerie?.imagesEtMedias?.length, null, 2),
  )

  const nomsHistoriquesTries =
    verrerie.nomsHistoriquesEtRaisonsSociales?.sort((a, b) => {
      const anneeA =
        a.anneeDebut || (a.periodeValidite ? parseInt(a.periodeValidite.split('-')[0]) : 0) || 0
      const anneeB =
        b.anneeDebut || (b.periodeValidite ? parseInt(b.periodeValidite.split('-')[0]) : 0) || 0
      return anneeA - anneeB
    }) || []

  // Extraire les données du lieu principal de manière sécurisée
  const lieu =
    verrerie.lieuPrincipal && typeof verrerie.lieuPrincipal === 'object'
      ? (verrerie.lieuPrincipal as LieuType)
      : null

  // Filtre les engagements de type "rôle de personnalité" (directeur, propriétaire...)
  const personnalitesEngagements =
    verrerie.engagements?.filter(
      (eng) =>
        eng.typeEngagement === 'role_personnalite' &&
  ...
  const verriersEngagements =
    verrerie.engagements?.filter(
      (eng) =>
        eng.typeEngagement === 'metier_verrier' &&
  ...
  const personnalitesGroupees: PersonneAvecEngagements<VerrierType>[] = (() => {
    ...
    for (const engagement of engagementsTries) {
      if (engagement.personneConcernee && typeof engagement.personneConcernee === 'object') {
        ...
        if (
          engagement.fonctionPersonnalite &&
          typeof engagement.fonctionPersonnalite === 'object' &&
          'nom' in engagement.fonctionPersonnalite
        ) {
  ...
  const verriersGroupes: PersonneAvecEngagements<VerrierType>[] = (() => {
    ...
    const map = new Map<string | number, PersonneAvecEngagements<VerrierType>>()
    const engagementsTries = [...verriersEngagements].sort((a, b) => {
      /* ... même logique de tri ... */
      const anneeA = a.dateDebutStructurée?.anneeDebutSort || 9999
  ...
  const pays =
    typeof verrerie.lieuPrincipal === 'object' && verrerie.lieuPrincipal.pays
  ...
  return (
    <div className="bg-cream text-blueGray-700 font-serif min-h-screen">
      <div className="max-w-screen-xl mx-auto py-12 md:py-16 px-4 md:px-8">
        <header className="mb-10 md:mb-16 text-center">
          ...
          {verrerie.nomsHistoriquesEtRaisonsSociales &&
            verrerie.nomsHistoriquesEtRaisonsSociales.length > 0 && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-x-12 gap-y-10">
          <main className="lg:col-span-2 space-y-8">
            {nomsHistoriquesTries.length > 0 && (
              <section id="noms-historiques" className="mb-12">
                {' '}
                {/* Ajout de mb-12 pour espacer de la section suivante */}
                ...
                <ul className="space-y-2 font-sans text-sm list-none pl-0">
                  {' '}
                  {/* list-none pour enlever les puces par défaut si non désiré */}
                  {nomsHistoriquesTries.map((item, index) => (
                    <li
                      key={item.id || `nom-hist-${index}`}
                      className="p-3 bg-white rounded-md shadow-sm border border-blueGray-100"
                    >
                      <div className="flex justify-between items-baseline">
                        ...
                        {item.typeDeNom && (
                          <span className="text-xs text-gold italic ml-2">
                            {item.typeDeNom === 'raison_sociale'
                              ? 'Raison Sociale'
                              : /* ... autres labels ... */ "Nom d'Usage"}
                          </span>
                        )}
                      </div>
                      {item.periodeValidite && (
                        <div className="text-xs text-blueGray-500 mt-0.5">
                          Période : {item.periodeValidite}
                        </div>
                      )}
                      {item.notes && (
                        <p className="mt-1 text-xs text-blueGray-600 whitespace-pre-line bg-blueGray-50 p-2 rounded">
                          {item.notes}
                        </p> // Notes avec un léger fond pour les distinguer
                      )}
                    </li>
                  ))}
                </ul>
            ...
            {verrerie.histoire && (
              <section
                id="histoire"
                className="scroll-mt-28 prose prose-lg max-w-none prose-headings:font-serif prose-headings:text-blueGray-800 prose-a:text-gold prose-strong:text-blueGray-800 prose-blockquote:border-gold prose-blockquote:text-blueGray-600"
              >
                <h2 className="text-3xl font-bold text-blueGray-800 mb-6 border-b-2 border-blueGray-200 pb-3 font-serif">
                  Histoire
                </h2>
            ...
            {verrerie.sourcesBibliographiques && verrerie.sourcesBibliographiques.length > 0 && (
              <section id="sources" className="mt-16">
                {' '}
                <h2 className="text-3xl font-bold text-blueGray-800 mb-8 border-b-2 border-blueGray-200 pb-4 font-serif">
                  Sources
                </h2>{' '}
                <ul className="list-disc list-inside space-y-2 text-blueGray-600 font-sans text-sm">
                  {' '}
                  {verrerie.sourcesBibliographiques.map((source) => (
                    <li key={source.id || Math.random().toString(36).substr(2, 9)}>
                      {' '}
                      <strong>{source.titre || 'Source non titrée'}</strong>{' '}
                      {source.auteur && ` par ${source.auteur}`}{' '}
                      {source.detailsPublication && ` (${source.detailsPublication})`}{' '}
                      {source.url && (
                        <a
                          href={source.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-gold hover:text-gold-dark hover:underline decoration-gold hover:decoration-gold-dark ml-1"
                        >
                          {' '}
                          [Lien]
                        </a>
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-24 space-y-6 rounded-xl p-1">
              ...
              <div className="bg-white p-6 rounded-xl shadow-xl border border-blueGray-100">
                <h3 className="text-2xl font-semibold text-blueGray-800 mb-5 font-serif">
                  Informations Clés
                </h3>
                <dl className="space-y-1 text-blueGray-600 font-sans text-sm">
                  {verrerie.fondateurs && verrerie.fondateurs.length > 0 && (
                    <div className="flex pt-1">
                      <strong className="font-bold text-blueGray-700 w-28 shrink-0">
                        Fondateur(s) :
                      </strong>
                      <div>
                        {verrerie.fondateurs.map((fondateur, index) => {
                          if (typeof fondateur === 'object' && fondateur !== null) {
                            const nomComplet =
                              `${fondateur.prenom || ''} ${fondateur.nom || ''}`.trim() ||
                              'Nom inconnu'
                            return (
                              <React.Fragment key={fondateur.id}>
                                {fondateur.slug ? (
                                  <Link
                                    href={`/verriers/${fondateur.slug}`}
                          ...
                          return (
                            <span key={fondateur as string | number} className="italic">
                              ID: {fondateur}
                            </span>
                          )
                        })}
                  ...
                  {!verrerie.periodeVerriere?.periodeActiviteTexte &&
                    (verrerie.periodeVerriere?.anneeFondationApprox ||
                      ...
                      <div className="flex pt-1">
                        ...
                        <span>
                          {verrerie.periodeVerriere?.anneeFondationApprox || '?'} -{' '}
                          {verrerie.periodeVerriere?.anneeFermetureApprox || 'Actuelle'}
                        </span>
                      </div>
                    )}
                  {verrerie.statutActuel && (
                    <div className="flex pt-1">
                      ...
                      <span>{getStatutLabel(verrerie.statutActuel)}</span>{' '}
                      {/* Bien utiliser la fonction, sinon on affiche la value/key */}
                    </div>
                  )}
                  {verrerie.notesStatutVestiges && (
                    <div className="pt-1">
                      <strong className="font-bold text-blueGray-700 block mb-0.5">
                        Notes statut :
                      </strong>
                      <TruncatedText
                        text={verrerie.notesStatutVestiges}
                        charLimit={250}
                        lineLimitForClamp={3}
                      />
                    </div>
                ...
                {verreriePourLaCarte.length > 0 && (
                  <div className="mt-5">
                    <strong className="font-bold font-sans text-blueGray-700 block mb-2">
                      Localisation :
                    </strong>
                    <div className="h-48 rounded-lg overflow-hidden border border-blueGray-200">
                      {' '}
                      {/* Hauteur réduite à h-48 */}
                      <VerrerieMap points={verreriePourLaCarte} disableMapAnimation={true} />
                    </div>
                  </div>
                )}
              </div>

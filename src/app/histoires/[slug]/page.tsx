...
import type {
  Histoire as HistoireType,
  Verrier as VerrierType,
  Verrery as VerrerieType,
} from '@/payload-types'
...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  ...
  const histoire = await getHistoire(slug)
...
export default async function HistoirePage({ params }: { params: any }) {
  ...
  const mapPoints: MapPoint[] = (histoire.verreriesLiees || [])
    .map((verrerie) => {
      ...
      if (
        typeof verrerie === 'object' &&
        verrerie.lieuPrincipal &&
        typeof verrerie.lieuPrincipal === 'object' &&
        verrerie.lieuPrincipal.coordonnees
      ) {
        return {
    ...
    <div className="bg-cream">
      <div className="max-w-screen-xl mx-auto px-4 py-12 md:py-16">
        ...
        <header className="mb-8 md:mb-12 max-w-4xl mx-auto">
          <h1 className="text-4xl sm:text-5xl font-bold text-blueGray-800 font-serif text-center">
            {histoire.title}
          </h1>
          {histoire.resume && (
            <p className="mt-4 text-lg text-blueGray-600 font-sans text-center">
              {histoire.resume}
            </p>
          )}
        </header>
            ...
            <div className="sticky top-28 space-y-6">
              ...
              {mapPoints.length > 0 && (
                <div className="bg-white p-6 rounded-lg shadow-md border">
                  <h3 className="text-xl font-semibold text-blueGray-800 mb-4 font-serif">
                    Lieux Mentionnés
                  </h3>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md border">
                  ...
                  <ul className="space-y-2">
                    {(histoire.personnesLiees as VerrierType[]).map((personne) => (
                      <li key={personne.id}>
                        <Link
                          href={`/verriers/${personne.slug}`}
                          className="text-gold hover:underline font-semibold text-sm"
                        >
                          {personne.nomComplet}
                        </Link>
                      </li>
